...
TEST_FIXTURE_FILE = f"{HOMES}/light-example.yaml"
...
@pytest.mark.parametrize(
    "config_yaml_fixture",
    [(f"{HOMES}/garage-door-example.yaml")],
)
...
@pytest.mark.parametrize(
    ...
    [
        (f"{HOMES}/light-on.yaml", "on"),
        (f"{HOMES}/light-off.yaml", "off"),
    ],
)
